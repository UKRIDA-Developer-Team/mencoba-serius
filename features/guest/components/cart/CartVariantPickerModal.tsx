...
type CartVariantPickerModalProps = {
    open: boolean;
    onClose: () => void;
    item: CartItem;
...
export default function CartVariantPickerModal({
    open,
    onClose,
    item,
    variants,
}: CartVariantPickerModalProps) {
    ...
    useEffect(() => {
        ...
        if (open) {
            dialog.showModal();
            setSelectedVariant(variants.find((v) => v.id === item.variantId) || null);
        } else {
            dialog.close();
        }
    }, [open, item.variantId, variants]);
    ...
    const finalPrice = selectedVariant?.priceOverride ?? item.price;
            ...
            <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-xl animate-step-in">
                ...
                <div className="p-4 space-y-2 max-h-60 overflow-y-auto">
                    {variants.map((variant) => {
                        ...
                        const hasPriceChange = variant.priceOverride !== null && variant.priceOverride !== item.price;
                ...
                <div className="p-4 border-t border-border flex items-center justify-between gap-3">
                    <div className="text-sm">
                        <span className="text-foreground/60">Harga baru: </span>
                    ...
                    <button
                        type="button"
                        disabled={!selectedVariant || selectedVariant.id === item.variantId}
